...
export class BookingService implements IBookingService {
	GetAllCompleted = (
		...
		const url: string = `${siteURL}/${SelectListInURL(
			ListNames.BookingList,
		)}/items?$select=Id&$filter=(${EBookingResponseKeys.CummuterRef} eq '${OfferId}') and (${
			EBookingResponseKeys.Destination
		} eq '${Destination}') and (${EBookingResponseKeys.Status} eq '${BookingStatus.ACCEPTED}')`;
		...
		return spHttpClient
			...
			.then((res) => {
				if (!res.ok) throw new Error('Cannot Fetch Competed Booking');
				return res.json();
			})
			.then((response) => {
	...
	GetIdOfBookingNotAcceptedUntillReachedLocationByOfferId = (
		OfferId: number,
		...
		const url: string = `${siteURL}/${SelectListInURL(
			...
			EBookingResponseKeys.Status
		} eq '${BookingStatus.REQUESTED}') and (${EBookingResponseKeys.Status} eq '${
			BookingStatus.REQUESTED
		}') and ((${EBookingResponseKeys.Source} eq '${ReachedLocation}') or (${
			EBookingResponseKeys.Destination
		} eq '${ReachedLocation}'))`;
		const options: ISPHttpClientOptions = {
			headers: {
	...
	GetAllByUserId = (UserId: number, spHttpClient: SPHttpClient): Promise<IBooking[]> => {
		const url: string = `${siteURL}/${SelectListInURL(
			BookingEntitySet,
		)}/items?$select=Id,BookingStatus,CommuterRefId,FarePrice,SourcePlace,TakerRefId,DestinationPlace,${
			EBookingResponseKeys.DateOfBooking
		},${EBookingResponseKeys.DestinationCoords},${EBookingResponseKeys.SourceCoords},${
			EBookingResponseKeys.SeatsRequired
		},Time&$filter=(${EBookingResponseKeys.PassengerRef} eq '${UserId}')&$orderby=Id desc`;
		const options: ISPHttpClientOptions = {
			headers: {
				Accept: 'application/json;odata=nometadata',
				Authorization: 'Bearer ' + AppSettings.tenantAPIKey,
		...
		return spHttpClient
			.get(url, SPHttpClient.configurations.v1, options)
			.then((res: SPHttpClientResponse) => {
				if (!res.ok) throw new Error(ErrorStatusText(res.status, res.statusText));
				return res.json();
			})
			.then((res): IBooking[] => {
	...
	GetAllOfferedRidesBooking = (
		OfferIds: number[],
		spHttpClient: SPHttpClient,
	): Promise<IBooking[]> => {
		let valuesArr: string = '';
		for (let i = 0; i < OfferIds.length; i++) {
			if (i == OfferIds.length - 1) valuesArr += `(CommuterRefId eq '${OfferIds[i]}')`;
			else valuesArr += `(CommuterRefId eq '${OfferIds[i]}') or `;
		}
		const url: string = `${siteURL}/${SelectListInURL(
			BookingEntitySet,
		)}/items?$select=Id,BookingStatus,CommuterRefId,FarePrice,SourcePlace,TakerRefId,SourceCoords,DestinationCoords,DestinationPlace,OData__DCDateCreated,${
			EBookingResponseKeys.SeatsRequired
		}&$filter=${valuesArr}&$orderby=Id desc`;
	...
	IsUnderBooking = (Id: number, spHttpClient: SPHttpClient): Promise<number> => {
		const url: string = `${siteURL}/${SelectListInURL(BookingEntitySet)}/items?select=Id&$filter=((${
			EBookingResponseKeys.Status
		} eq '${BookingStatus.REQUESTED}') or (${EBookingResponseKeys.Status} eq '${
			BookingStatus.ACCEPTED
		}')) and (${EBookingResponseKeys.PassengerRef} eq '${Id}')`;
		const options: ISPHttpClientOptions = {
			headers: {
				Accept: 'application/json;odata=nometadata',
				Authorization: 'Bearer ' + AppSettings.tenantAPIKey,
	...
	GetCountByLocation = (
		...
		const filterColumn: string =
			LocationType == ELocationType.SOURCE
				? EBookingResponseKeys.Source
				: EBookingResponseKeys.Destination;
		const url: string = `${siteURL}/${SelectListInURL(ListNames.BookingList)}/items?$select=Id,${
			EBookingResponseKeys.SeatsRequired
		}&$filter=(${EBookingResponseKeys.CummuterRef} eq '${OfferId}') and ((${
			EBookingResponseKeys.Status
		} eq '${BookingStatus.ACCEPTED}') or (${EBookingResponseKeys.Status} eq '${
			BookingStatus.COMPLETED
		}')) and (${filterColumn} eq '${Location}')`;
		const options: ISPHttpClientOptions = {
			headers: {
				Accept: 'application/json;odata=nometadata',
				Authorization: 'Bearer ' + AppSettings.tenantAPIKey,
	...
	GetById = (BookId: number, spHttpClient: SPHttpClient): Promise<IBooking> => {
		const url: string = `${siteURL}/${SelectListInURL(
			BookingEntitySet,
		)}/items('${BookId}')?$select=Id,SourceCoords,DestinationCoords,Time,BookingStatus,CommuterRefId,FarePrice,SourcePlace,TakerRefId,DestinationPlace,OData__DCDateCreated,${
			EBookingResponseKeys.SeatsRequired
		}`;
		const options: ISPHttpClientOptions = {
			headers: {
